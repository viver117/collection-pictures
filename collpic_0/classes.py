...
class Field():
  def __init__(self):
    ...
    if (SIZE_FIELD == 3): address = os.path.join("pics", "300x300")
    if (SIZE_FIELD == 4): address = os.path.join("pics", "400x400")
    if (SIZE_FIELD == 5): address = os.path.join("pics", "500x500")
	
    for i in range(0, SIZE_FIELD**2):
      self.pics.append(pygame.image.load(os.path.join(address, str(i) + ".jpg"))) # Заполнение полей в поле
